...
void FC_FUNC_(prepare_fields_elastic_adj_dev,
  ...
  //xyw 2025/3/15
  print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_c11_kl),size*sizeof(realw)),9001);
  print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_c13_kl),size*sizeof(realw)),9002);
  print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_c15_kl),size*sizeof(realw)),9003);
  print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_c33_kl),size*sizeof(realw)),9004);
  print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_c35_kl),size*sizeof(realw)),9005);
  print_CUDA_error_if_any(cudaMalloc((void**)&(mp->d_c55_kl),size*sizeof(realw)),9006);
  ...
  //xyw 2025/3/15
  print_CUDA_error_if_any(cudaMemset(mp->d_c11_kl,0,size*sizeof(realw)),9007);
  print_CUDA_error_if_any(cudaMemset(mp->d_c13_kl,0,size*sizeof(realw)),9008);
  print_CUDA_error_if_any(cudaMemset(mp->d_c15_kl,0,size*sizeof(realw)),9009);
  print_CUDA_error_if_any(cudaMemset(mp->d_c33_kl,0,size*sizeof(realw)),9010);
  print_CUDA_error_if_any(cudaMemset(mp->d_c35_kl,0,size*sizeof(realw)),9011);
  print_CUDA_error_if_any(cudaMemset(mp->d_c55_kl,0,size*sizeof(realw)),9012);
...
TRACE("prepare_cleanup_device");
  ...
  if (*ELASTIC_SIMULATION) {
    ...
    if (mp->simulation_type == 3) {
      ...
      //xyw 2025/3/15
      cudaFree(mp->d_c11_kl);
      cudaFree(mp->d_c13_kl);
      cudaFree(mp->d_c15_kl);
      cudaFree(mp->d_c33_kl);
      cudaFree(mp->d_c35_kl);
      cudaFree(mp->d_c55_kl);
