...
!========================================================================
  ...
  !xyw 2025/3/13
  else if ((trim(SAVE_MODEL) == 'binary_voigt'))  then
    ! binary and GLL format
    print *, 'c11store(1,1,1)', c11store(1,1,1)
    print *, 'c11store(1,1,200)', c11store(1,1,200)
    print *, 'c12store(1,1,200)', c12store(1,1,200)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_rho.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_rho.bin')
    write(172) rho_save
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c11.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c11.bin')
    write(172) c11store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c13.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c13.bin')
    write(172) c13store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c15.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c15.bin')
    write(172) c15store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c33.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c33.bin')
    write(172) c33store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c35.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c35.bin')
    write(172) c35store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c55.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c55.bin')
    write(172) c55store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c12.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c12.bin')
    write(172) c12store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c23.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c23.bin')
    write(172) c23store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_c25.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank,'Error opening model file proc**_c25.bin')
    write(172) c25store
    close(172)
    write(filename,'(a,i6.6,a)') trim(IN_DATA_FILES)//'proc',myrank,'_x.bin'
    open(unit=172,file=filename,status='unknown',form='unformatted',iostat=ier)
