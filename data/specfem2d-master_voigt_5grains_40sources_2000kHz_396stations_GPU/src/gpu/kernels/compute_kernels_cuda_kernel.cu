...
__global__ void compute_kernels_cudakernel(const int* ispec_is_elastic,
  ...
  if (active) {
    ...
    if (ispec_is_anisotropic[ispec]) {
      //TRACE("compute_anisotropic_gpu 33333333333333333333333333333333333333333333333333333333333333333");
      //printf("ispec = %d, ij = %d, iglob = %d, dsxx = %f, b_dsxx = %f\n", ispec, ij, iglob, dsxx, b_dsxx);
      c11_kl[ij_ispec] += dsxx * b_dsxx;
      //printf("c11_kl[%d] = %f\n", ij_ispec, c11_kl[ij_ispec]);
      c13_kl[ij_ispec] += dsxx * b_dszz + dszz * b_dsxx;
      c15_kl[ij_ispec] += 2 * (dsxx * b_dsxz + b_dsxx * dsxz);
      c33_kl[ij_ispec] += dszz *b_dszz ;
      c35_kl[ij_ispec] += 2 * (dszz * b_dsxz + b_dszz * dsxz);
      c55_kl[ij_ispec] += 4 * dsxz * b_dsxz;
    }
  }
}
